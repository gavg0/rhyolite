...
<div
  class={`flex flex-col h-full w-full border border-overlay2 rounded-lg text-text ${classProp}`}
>
  <div class="flex justify-center {flagToolbarVisibility ? 'border-b ' : ''}">
    <button
      class="w-full flex justify-center hover:bg-text/10"
      onclick={ContentEditorStore.toggleToolbarVisibility}
    >
  ...
  <div class="border-b px-3">
    <div class:hidden={!flagToolbarVisibility}>
      <div class="flex items-center space-x-1 rtl:space-x-reverse flex-wrap">
        ...
        <Dropdown
          bind:open={textSizeDropdownOpen}
          class="w-72 rounded bg-surface0 p-2 shadow "
        >
          {#each [{ size: "16px", label: "16px (Default)", className: "text-base" }, { size: "12px", label: "12px (Tiny)", className: "text-xs" }, { size: "14px", label: "14px (Small)", className: "text-sm" }, { size: "18px", label: "18px (Lead)", className: "text-lg" }, { size: "24px", label: "24px (Large)", className: "text-2xl" }, { size: "36px", label: "36px (Huge)", className: "text-4xl" }] as { size, label, className }}
        ...
        <Tooltip triggeredBy="#toggleTextSizeButton">Text size</Tooltip>
        <button
        ...
        <Dropdown
          bind:open={colorDropdownOpen}
          class="w-48 rounded bg-surface0 p-2 shadow "
        >
          <div
            class="grid grid-cols-6 gap-2 group mb-3 items-center p-1.5 rounded-lg hover:bg-gray-100"
          >
            <input
              type="color"
              id="color"
              ...
              class="border-gray-200 border bg-surface0 rounded-md p-px px-1 hover:bg-surface0 group-hover:bg-surface0 w-full h-8 col-span-3"
            />
            <label
              for="color"
              class="text-surface0 text-sm font-medium col-span-3 group-hover:text-gray-900"
              >Pick a color</label
            >
          </div>
          <div class="grid grid-cols-6 gap-1 mb-3">
            {#each [{ color: "#1A56DB", label: "Blue" }, { color: "#0E9F6E", label: "Green" }, { color: "#FACA15", label: "Yellow" }, { color: "#F05252", label: "Red" }, { color: "#FF8A4C", label: "Orange" }, { color: "#0694A2", label: "Teal" }, { color: "#B4C6FC", label: "Light indigo" }, { color: "#8DA2FB", label: "Indigo" }, { color: "#5145CD", label: "Purple" }, { color: "#771D1D", label: "Brown" }, { color: "#FCD9BD", label: "Light orange" }, { color: "#99154B", label: "Bordo" }, { color: "#7E3AF2", label: "Dark Purple" }, { color: "#CABFFD", label: "Light" }, { color: "#D61F69", label: "Dark Pink" }, { color: "#F8B4D9", label: "Pink" }, { color: "#F6C196", label: "Cream" }, { color: "#A4CAFE", label: "Light Blue" }, { color: "#5145CD", label: "Dark Blue" }, { color: "#B43403", label: "Orange Brown" }, { color: "#FCE96A", label: "Light Yellow" }, { color: "#1E429F", label: "Navy Blue" }, { color: "#768FFD", label: "Light Purple" }, { color: "#BCF0DA", label: "Light Green" }, { color: "#EBF5FF", label: "Sky Blue" }, { color: "#16BDCA", label: "Cyan" }, { color: "#E74694", label: "Pink" }, { color: "#83B0ED", label: "Darker Sky Blue" }, { color: "#03543F", label: "Forest Green" }, { color: "#111928", label: "Black" }, { color: "#4B5563", label: "Stone" }, { color: "#6B7280", label: "Gray" }, { color: "#D1D5DB", label: "Light Gray" }, { color: "#F3F4F6", label: "Cloud Gray" }, { color: "#F3F4F6", label: "Cloud Gray" }, { color: "#F9FAFB", label: "Heaven Gray" }] as { color, label }}
              <button
                type="button"
                data-hex-color={color}
                onclick={onClickColorHex}
          ...
          <button
            type="button"
            onclick={resetColor}
            class="py-1.5 text-sm font-medium text-text focus:outline-none bg-white rounded-lg hover:bg-gray-100 hover:text-gray-900 focus:z-10 focus:ring-4 focus:ring-gray-100 w-full"
            >Reset color</button
          >
        </Dropdown>
        <Tooltip triggeredBy="#toggleTextColorButton">Text size</Tooltip>
        <button
        ...
        <Dropdown
          bind:open={fontFamilyDropdownOpen}
          class="w-72 rounded bg-white p-2 shadow  text-sm font-medium"
        >
          {#each [{ name: "Default", fontFamily: "Inter, ui-sans-serif", style: "" }, { name: "Arial", fontFamily: "Arial, sans-serif", style: "font-family: Arial, sans-serif;" }, { name: "Courier New", fontFamily: "'Courier New', monospace", style: "font-family: 'Courier New', monospace;" }, { name: "Georgia", fontFamily: "Georgia, serif", style: "font-family: Georgia, serif;" }, { name: "Lucida Sans Unicode", fontFamily: "'Lucida Sans Unicode', sans-serif", style: "font-family: 'Lucida Sans Unicode', sans-serif;" }, { name: "Tahoma", fontFamily: "Tahoma, sans-serif", style: "font-family: Tahoma, sans-serif;" }, { name: "Times New Roman", fontFamily: "'Times New Roman', serif", style: "font-family: 'Times New Roman', serif;" }, { name: "Trebuchet MS", fontFamily: "'Trebuchet MS', sans-serif", style: "font-family: 'Trebuchet MS', sans-serif;" }, { name: "Verdana", fontFamily: "Verdana, sans-serif", style: "font-family: Verdana, sans-serif;" }] as { name, fontFamily, style }}
            <DropdownItem>
              <button
                onclick={onSelectFontFamily}
                data-font-family={fontFamily}
                type="button"
                class="flex justify-between items-center w-full text-sm rounded px-3 py-2 hover:bg-gray-100 text-gray-900"
                {style}
              >
                {name}
              </button>
        ...
        <Tooltip triggeredBy="#toggleFontFamilyButton">Font family</Tooltip>
        <div class="px-1">
      ...
      <div class="flex items-center gap-2 pt-2 flex-wrap">
        <button
          ...
          class="flex custom-btn-toolbar items-center justify-center rounded-lg px-3 py-1.5 text-sm font-medium focus:z-10 focus:outline-none focus:ring-0 focus:ring-surface0"
          type="button"
        >
          Format
          <svg
        ...
        <!-- Heading Dropdown -->
        <Dropdown
          bind:open={typographyDropdownOpen}
          class="w-72 rounded bg-base p-2 shadow text-sm font-medium"
        >
          {#each [{ label: "Paragraph", keys: ["Cmd", "Alt", "0"], headingLevel: "0", onclick: onSelectParagraph }, { label: "Heading 1", keys: ["Cmd", "Alt", "1"], headingLevel: "1", onclick: onSelectHeading }, { label: "Heading 2", keys: ["Cmd", "Alt", "2"], headingLevel: "2", onclick: onSelectHeading }, { label: "Heading 3", keys: ["Cmd", "Alt", "3"], headingLevel: "3", onclick: onSelectHeading }, { label: "Heading 4", keys: ["Cmd", "Alt", "4"], headingLevel: "4", onclick: onSelectHeading }, { label: "Heading 5", keys: ["Cmd", "Alt", "5"], headingLevel: "5", onclick: onSelectHeading }, { label: "Heading 6", keys: ["Cmd", "Alt", "6"], headingLevel: "6", onclick: onSelectHeading }] as { label, keys, headingLevel, onclick }}
            <button
              type="button"
              {onclick}
              data-heading-level={headingLevel}
              class="flex justify-between items-center w-full text-base rounded px-3 py-2 custom-btn-toolbar my-0.5"
              >{label}
              <span class="space-x-1">
                {#each keys as key}
                  <kbd
                    class="px-1 py-0.5 text-xs font-semibold text-text/70 bg-surface0 rounded-lg"
                    >{key}</kbd
                  >
                {/each}
              </span>
        ...
        <div class="ps-1.5">
          <span class="block w-px h-4 bg-surface2"></span>
        </div>
        ...
        <ToolbarButton
          onclick={() => toggleMark("toggleBlockquote")}
        ...
        <ToolbarButton
          onclick={() => toggleMark("setHorizontalRule")}
