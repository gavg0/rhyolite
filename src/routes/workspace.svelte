<script lang="ts">
    ...
    async function deleteDocument(): Promise<void> {
        try {
            ...
            const nextDoc: DocumentData | null = await invoke(
                "delete_document",
                { id: currentId },
            );
            currentTabs = await getTabs();
...
<main>
    <div
        ...
        class="fixed flex bg-base top-[0px] w-full h-[40px] select-none justify-between px-1 z-10"
        role="tablist"
        aria-label="Document tabs"
    >
        <div
            data-tauri-drag-region
            class="flex flex-row items-center h-full px-4 ml-4 flex-grow"
        >
        ...
        <div class="flex flex-none flex-row items-center h-full">
            <button
                class="titlebar-button h-full px-3 cursor-pointer hover:bg-surface2"
            ...
            <button
                class="titlebar-button h-full px-3 cursor-pointer hover:bg-red-700"
                id="titlebar-close"
